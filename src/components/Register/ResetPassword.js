...
const ResetPassword = () => {
  ...
  useEffect(() => {
    const asyncFc = async () => {
      ...
      try {
        let result = await api.post(`forgotpassword/checkvalidate`, { email: decodeToken.email, token: decodeToken.token });
  ...
  const onSubmit = async (data) => {
    try {
      let result = await api.post(`forgotpassword/reset`, { email: resetState.email, password });
      if (result) {
